...
import init, { generate_sudoku } from './rust-wasm/sudoku-wasm/pkg/sudoku_wasm.js'
...
function drawHighlights(p, i, spacing) {
    ...
    if(i == 11) changed_highlights_3d = false
}
    ...
    for (let i = 1; i <= 12; i++) {
        new p5((p) => {
            ...
            p.setup = () => {
                ...
                spacing = container.clientWidth / 12
                canvas.parent(`p5-${i}`);
                p.noStroke();
                ...
                if(mouse_selection[2] + 1 === i) {
                    ...
                    if (p.keyCode === p.DOWN_ARROW) {
                        ...
                        if(mouse_selection.length > 0) {
                            ...
                            if(mouse_selection[1] > 11) {
                                mouse_selection[1] = 0
                                mouse_selection[2] = mouse_selection[2] + 1
                                if(mouse_selection[2] > 11) {
                                    mouse_selection[2] = 0
                                }
                            }
                        }
                        ...
                        if(mouse_selection.length > 0) {
                            ...
                            if(mouse_selection[1] < 0) {
                                mouse_selection[1] = 11
                                mouse_selection[2] = mouse_selection[2] - 1
                                if(mouse_selection[2] < 0) {
                                    mouse_selection[2] = 11
                                }
                            }
                        }
...
async function run() {
    await init();
    window.finished_sudoku = generate_sudoku()
    window.sudoku = structuredClone(window.finished_sudoku)
    window.player_game = structuredClone(window.finished_sudoku)
    console.log(window.finished_sudoku)
